...
class Database:
    ...
    def sample_query(self):
        query = "SELECT first_name, last_name FROM {TBL_USR} WHERE last_name LIKE 'M%';".format(
            TBL_USR=USERS)
        ...
        query = "SELECT u.last_name as last_name, " \
                ...
                "WHERE u.id=a.user_id AND u.last_name LIKE 'M%';".format(
                    TBL_USR=USERS, TBL_ADDR=ADDRESSES)
        self.print_all_data(query=query)
    ...
    def sample_insert(self):
        query = "INSERT INTO {}(id, first_name, last_name) VALUES (3, 'Terrence','Jordan');".format(
            USERS)
        self.execute_query(query)
        self.print_all_data(USERS)

    def sample_update(self):
        query = "UPDATE {} set first_name='XXXX' WHERE id={id}".format(
            USERS, id=3)
        self.execute_query(query)
        self.print_all_data(USERS)
